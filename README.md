...
`📊 وضعیت` · `▶️ استارت` · `🤖 مدل` — همیشه در دسترس، بدون تایپ.
(دکمهٔ «⌨️ کیبورد» در منو هم آن را دوباره نشان می‌دهد.)

## 🎛 رابط دکمه‌ای (inline)
`/menu` (یا `/start`) منوی دکمه‌ای باز می‌کند:
- ردیف اول: **📊 وضعیت** (چپ) · **▶️ استارت** (وسط) · **🤖 مدل** (راست).
- **⚙️ تنظیمات** یک زیرمنوی واقعی است: مود، تبلیغات، مدل و **⏰ هشدار انقضای
  سشن** (خاموش / ۲ / ۵ / ۱۰ دقیقه).
- **▶️ استارت** اگر سشنی نباشد سشن تازه می‌سازد و اگر فعال باشد وضعیتش را نشان می‌دهد.
- یک **دکمهٔ ثابت تایمر در پایین منو** همیشه دیده می‌شود: «⏳ سشن: ۴۲ دقیقه»
  یا «⏳ سشن بسته — ▶️ استارت»؛ با زدنش وضعیت و دکمهٔ تمدید می‌آید.
- **💬 سشن‌ها** دکمهٔ سوییچ و حذف (با تأیید) دارد.
...
| ⌨️ کیبورد | نمایش دوبارهٔ کیبورد ثابت پایین |
