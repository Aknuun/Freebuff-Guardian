...
- افزودن: `/account add friend1` و بعد فرستادن محتوای `credentials.json` آن اکانت.
...
1. هر نفر روی سرور خودش `freebuff login` می‌کند و فایل
   `~/.config/manicode/credentials.json` را برمی‌دارد.
2. در ربات: `/account add friend1`
3. محتوای کامل همان `credentials.json` را پیست و بفرست.
4. از منوی **👤 اکانت** (یا `/account`) بین حساب‌ها سوییچ کن.

نکته‌ها:
- اکانت‌ها در پوشهٔ `accounts/` ذخیره می‌شوند و در `.gitignore` هستند (توکن‌ها لو نمی‌روند).
