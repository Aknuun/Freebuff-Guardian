...
- افزودن: `/account add friend1` و بعد یا **ورود با وب** (لینک لاگین، بدون سرور)
  یا پیست `credentials.json`.
...
`/account add <name>` را بزن و یکی از دو روش را انتخاب کن:

### روش ۱: 🌐 ورود با وب (بدون نیاز به سرور یا نصب CLI)
1. `/account add friend1`
2. دکمهٔ **🌐 ورود با وب** را بزن.
3. ربات یک لینک لاگین (`.../login?auth_code=...`) می‌دهد؛ همان لینک را
   باز کن، در سایت فری‌باف لاگین کن و تأیید کن.
4. ربات خودش تأیید را می‌فهمد و اکانت را ذخیره می‌کند.
...
2. `/account add friend1` → دکمهٔ **📋 پیست credentials.json** → محتوا را بفرست.

### تعویض حساب
از منوی **👤 اکانت** (یا `/account`) روی اکانت بزن تا فعال شود.
